...
# CẤU HÌNH
# =========================
TARGET_URL = "https://hygenie.io/"
PLAYER_ORIGIN_FALLBACK = "https://hygenie.io"
...
SCANNER_VERSION = "1.0.2-HYGENIE-MINIMAL-TWO-FILE"
...
MAX_OUTPUT_STREAMS_PER_MATCH = read_env_int("HYGENIE_MAX_OUTPUT_STREAMS_PER_MATCH", 4, minimum=1, maximum=8)
HEADLESS = True
...
def probe_stream_sync(
    ...
    if not effective_origin:
        ...
        if parsed_ref.netloc.endswith("hygenie.io"):
...
async def collect_home_links(context: BrowserContext) -> list[dict[str, Any]]:
    ...
    print(f"👉 Đang mở trang chủ Hygenie: {TARGET_URL}")
    try:
        ...
        result = await page.evaluate(r"""({targetHost}) => {
            const clean = (v) => String(v || "").replace(/\s+/g," ").trim();
            const abs = (v) => { try { return new URL(v, location.href).href; } catch (_) { return ""; } };
            const isMatch = (v) => { try {
                const u = new URL(v, location.href);
                return (u.hostname === targetHost || u.hostname.endsWith("." + targetHost)) && u.pathname.includes("/truc-tiep/") && /-vao-luc-|\bvs\b|-vs-/i.test(decodeURIComponent(u.pathname));
            ...
            (html.match(/(?:https?:\/\/[^"'<>\s]+)?\/truc-tiep\/[^"'<>\s?]+(?:\?blv=\d+)?/gi)||[]).forEach((href)=>add(href,null,""));
            return {items,diagnostics:{title:document.title||"",anchors:document.querySelectorAll("a[href]").length,html_length:html.length}};
        }""", {"targetHost": urlparse(TARGET_URL).hostname or "hygenie.io"})
        ...
        print(
            f"ℹ️ Hygenie: title={diagnostics.get('title','')!r} | "
...
async def main() -> None:
    ...
    print(f"🧞 KHỞI ĐỘNG HYGENIE STREAM SCANNER - {SCANNER_VERSION}", flush=True)
    print(
        "ℹ️ Test riêng một trận:\n"
        '   python main.py "https://hygenie.io/truc-tiep/le-be-mac-vs-luong-son-tv-vao-luc-0030-20-07-2026/?blv=246"'
    )
    print(
