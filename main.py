...
# Đã update User-Agent y hệt bản F12 của bác (Chrome 147)
UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/147.0.0.0 Safari/537.36"
BLACKLIST = ["ufc", "mma", "tennis", "quần vợt", "bóng rổ", "cầu lông", "bóng chuyền", "esport", "atp", "wta", "itf", "bóng bàn", "đua xe", "billiard", "wwe", "taro daniel", "brancaccio"]
...
def clean_title_basic(raw_text):
    ...
    for j in junk: raw_text = re.sub(r'(?i)\b' + j + r'\b', '', raw_text)
...
def extract_streams(url):
    try:
        ...
        for m in re.finditer(r'["\'](?:name|title|blv)["\']\s*:\s*["\']([^"\']+)["\'].*?["\'](?:url|src|link)["\']\s*:\s*["\']([^"\']+)["\']', html, re.I):
            ...
            try: blv_map[u] = bytes(m.group(1), 'utf-8').decode('unicode_escape')
            except: blv_map[u] = m.group(1)
            
        for m in re.finditer(r'["\'](?:url|src|link)["\']\s*:\s*["\']([^"\']+)["\'].*?["\'](?:name|title|blv)["\']\s*:\s*["\']([^"\']+)["\']', html, re.I):
            u = m.group(1).replace('\\/', '/').replace('\\', '')
            try: blv_map[u] = bytes(m.group(2), 'utf-8').decode('unicode_escape')
            except: blv_map[u] = m.group(2)
            ...
            if d_link:
                ...
                if '.m3u8' not in d_link and '.flv' not in d_link:
                    try:
                        ...
                        for l in re.findall(r'(https?://[^\s"\'<>]*\.(?:m3u8|flv)[^\s"\'<>]*)', s_res.text): add_stream(l, btn_name)
                    except: pass
...
def main():
    ...
    try:
        ...
        for a_tag in soup.find_all('a', href=True):
            ...
            if '/truc-tiep/' in href:
                ...
                if not any(m['url'] == full_link for m in matches):
                    matches.append({'url': full_link, 'clean_title': clean_title, 'time': time_tag, 'logo': logo, 'sort': sort_val})
        
        matches.sort(key=lambda x: x['sort'])
        playlist = "#EXTM3U\n"
        print("✅ Đang tổng hợp Link...")
        
        # Bóc base_domain chuẩn: https://bunchatv4.net
        ...
        for m in matches:
            ...
            if streams:
                for s in streams:
                    ...
                    # Bơm User-Agent bằng lệnh độc lập để an toàn cho URL
                    playlist += f'#EXTVLCOPT:http-user-agent={UA}\n'
                    
                    # ÉP KHUÔN HEADER: Nối thẳng Referer và Origin vào đuôi link bằng dấu "|"
                    final_url = s["url"]
                    if "|" not in final_url:
                        final_url += f"|Referer={base_domain}/&Origin={base_domain}"
        ...
        with open("buncha_live.m3u", "w", encoding="utf-8") as f:
            f.write(playlist)
        print(f"🎉 HOÀN TẤT! Áo giáp đã mặc đủ, sẵn sàng qua mặt máy chủ.")
